...
function renderScrapersTable(scrapers) {
    ...
    scrapers.forEach(scraper => {
        html += `
            <tr>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${escapeHTML(scraper.config.name)}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${escapeHTML(scraper.config.target_url)}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${new Date(scraper.created_at).toLocaleString()}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button onclick="loadView('scraper_details', this.dataset.id)" data-id="${escapeHTML(scraper.id)}" class="text-indigo-600 hover:text-indigo-900">View Details</button>
                </td>
            </tr>
        `;
    });
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <dl class="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                    <div class="sm:col-span-1">
                        ...
                        <dd class="mt-1 text-sm text-gray-900">${escapeHTML(scraper.config.name)}</dd>
                    </div>
                    <div class="sm:col-span-1">
                        <dt class="text-sm font-medium text-gray-500">Target URL</dt>
                        <dd class="mt-1 text-sm text-gray-900">${escapeHTML(scraper.config.target_url)}</dd>
                    </div>
                    <div class="sm:col-span-2">
                        <dt class="text-sm font-medium text-gray-500">Selectors</dt>
                        <dd class="mt-1 text-sm text-gray-900"><pre class="bg-gray-100 p-2 rounded text-xs">${escapeHTML(JSON.stringify(scraper.config.selectors, null, 2))}</pre></dd>
                    </div>
                </dl>
            </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ${runs && runs.map(run => `
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-red-500 truncate max-w-xs">${escapeHTML(run.error_message || '-')}</td>
                        </tr>
    ...
    return alerts.map(alert => `
        <div class="mb-3 p-3 bg-red-50 border-l-4 border-red-500 rounded">
            <div class="flex justify-between">
                <span class="text-xs font-bold text-red-800 uppercase">${escapeHTML(alert.type)}</span>
                <span class="text-xs text-gray-500">${new Date(alert.timestamp).toLocaleTimeString()}</span>
            </div>
            <p class="text-sm text-red-700 mt-1">${escapeHTML(alert.message)}</p>
        </div>
    `).join('');
}
